...
namespace DS.Windows
{
    ...
    public class DSGraphView : GraphView
    {
        ...
        //allows copy pasting
        private List<ISelectable> copiedElements = new List<ISelectable>();
        private Vector2 copyPasteOffset = new Vector2(10, 10);
        ...
        private void AddManipulators()
        {
            ...
            this.AddManipulator(new KeyboardPanManipulator());
        ...
        private void AddCopyPasteManipulators()
        {
            // Register copy-paste callbacks
        ...
        /// <summary>
        /// Copies the currently selected nodes and groups
        /// </summary>
        private void CopySelection()
        {
            ...
            {
                // Only copy nodes and groups, not edges or other elements
                if (selectedElement is DSNode || selectedElement is DSGroup)
                {
            ...
            {
                Debug.Log($"Copied {copiedElements.Count} elements");
            }
        }

        /// <summary>
        /// Pastes the copied elements with a slight offset
        /// </summary>
        private void PasteSelection()
        {
            if (copiedElements.Count == 0) return;

            // Clear current selection
            ClearSelection();

            var mousePosition = GetLocalMousePosition(Event.current.mousePosition);
            var newElements = new List<ISelectable>();

            foreach (var element in copiedElements)
            {
                if (element is DSNode originalNode)
                {
                    var newNode = CreateNodeFromCopy(originalNode, mousePosition);
                    newElements.Add(newNode);
                    AddToSelection(newNode);
                }
                else if (element is DSGroup originalGroup)
                {
                    var newGroup = CreateGroupFromCopy(originalGroup, mousePosition);
                    newElements.Add(newGroup);
                    AddToSelection(newGroup);
                }
            }

            // Update the copied elements to the new ones for repeated pasting
            copiedElements = newElements;
            Debug.Log($"Pasted {newElements.Count} elements");
        }

        /// <summary>
        /// Cuts the selection (copy + delete)
        /// </summary>
        ...
        /// <summary>
        /// Creates a new node as a copy of an existing node
        /// </summary>
        private DSNode CreateNodeFromCopy(DSNode originalNode, Vector2 pastePosition)
        {
            // Calculate position with offset
            var originalPosition = originalNode.GetPosition().position;
            var newPosition = pastePosition + (originalPosition - GetCenterOfCopiedElements()) + copyPasteOffset;

            // Create new node of the same type
            DSNode newNode = CreateNode(
                originalNode.DialogueName + "Copy",
                originalNode.DialogueType,
                newPosition,
                false
            );
            ...
            // Copy choices (deep copy)
            newNode.Choices = new List<DSChoiceSaveData>();
            foreach (var originalChoice in originalNode.Choices)
            {
                newNode.Choices.Add(new DSChoiceSaveData
                {
                    Text = originalChoice.Text,
                    NodeID = "" // Clear connections for copied nodes
                });
            }
            ...
            // Handle grouping
            if (originalNode.Group != null)
            {
                // If copying within same graph, add to same group
                newNode.Group = originalNode.Group;
                originalNode.Group.AddElement(newNode);
            }
            else
            {
                AddUngroupedNode(newNode);
            }
        ...
        /// <summary>
        /// Creates a new group as a copy of an existing group
        /// </summary>
        private DSGroup CreateGroupFromCopy(DSGroup originalGroup, Vector2 pastePosition)
        {
            var originalPosition = originalGroup.GetPosition().position;
            var newPosition = pastePosition + (originalPosition - GetCenterOfCopiedElements()) + copyPasteOffset;

            DSGroup newGroup = CreateGroup(originalGroup.title + "Copy", newPosition);
        ...
        /// <summary>
        /// Calculates the center point of all copied elements for relative positioning
        /// </summary>
        private Vector2 GetCenterOfCopiedElements()
        {
            if (copiedElements.Count == 0) return Vector2.zero;
            ...
            foreach (var element in copiedElements)
            {
                ...
                {
                    center += node.GetPosition().position;
                    count++;
                }
                else if (element is DSGroup group)
                {
                    center += group.GetPosition().position;
                    count++;
                }
            }
